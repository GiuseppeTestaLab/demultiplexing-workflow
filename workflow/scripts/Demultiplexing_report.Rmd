...
```{r, collapse=TRUE}
params$Dataset <- snakemake@params[["dataset"]]
params$Demuxlet_V1 <- snakemake@input[["demuxlet_v1"]]
params$Demuxlet_V2 <- snakemake@input[["demuxlet_v2"]]
params$SoC <- snakemake@input[["soc"]]
params$Vireo <- snakemake@input[["vireo"]]
params$SCanSNP <- snakemake@input[["scansnp"]]
params$BarcodeMap <- snakemake@params[["barcode_map"]]
params$FilteredFeaturesPath <- snakemake@params[["filtered_matrix"]]
params$outFile <- snakemake@output[["combined_results"]]
```
####  Analysis is performed on `r params$Dataset`.
...
# SoC
SoC <- read.csv(params$SoC, header = T, sep = "\t",stringsAsFactors=F)
SoC$SoCID <- ifelse(SoC$status == "singlet", SoC$assignment, SoC$status)

# Vireo
Vireo <- read.csv(params$Vireo, header = T, sep = "\t",stringsAsFactors=F) %>% dplyr::rename(barcode=cell, VireoID=donor_id)
Vireo <-  Vireo[,c("barcode","VireoID")]

# Demuxlet V1
Demuxlet_V1 <- read.csv(params$Demuxlet_V1, header = T, sep = "\t",stringsAsFactors=F) %>% dplyr::rename(barcode=BARCODE) %>%
...
# Demuxlet V2
Demuxlet_V2 <- read.csv(params$Demuxlet_V2, header = T, sep = "\t",stringsAsFactors=F)  %>% dplyr::rename(barcode=BARCODE) %>%
...
# ScanSnp
Scansnp <- read.csv(params$SCanSNP, sep = "\t", header = T,stringsAsFactors=F) %>% dplyr::rename(barcode=Barcode, SCanSNPID=ID)
...
#mergedIDs  <- merge(mergedIDs, Demuxlet_V2 ,by = "barcode")
```
...
if (params$FilteredFeaturesPath != "None"){

  par(mfrow=c(5,1))

  sc.data <- Read10X(data.dir = params$FilteredFeaturesPath)
...
if (params$BarcodeMap != "None"){

  BarcodeMap <- params$BarcodeMap
...
bestComb <- unlist(strsplit(as.character(tail(AgreementDF[order(AgreementDF$AgreementRatio),"Software"],1)), "_VS_"))
...
IDagreementRatios[["shared_overall"]] <- round(as.numeric(tail(AgreementDF[order(AgreementDF$AgreementRatio),"AgreementRatio"],1))/100,2)

IDagreementRatios[["best_agreement"]] <- as.character(tail(AgreementDF[order(AgreementDF$AgreementRatio),"Software"],1))
...
for (tool in toolsList){
  ...
  toolRow <- data.frame("Software"=tool, "DBLsRate"=ToolDBLsRatio, "TotalDroplets"= DSsize, "Dataset"=params$Dataset, "DStype"=DStype)
...
write.table(MetricsDF, file=params$outFile, sep = "\t", row.names = F, col.names = T,quote = F)
